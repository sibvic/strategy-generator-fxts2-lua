        ...
        if (last_serial ~= current_serial) then
            ...
            if (instance.parameters.close_on_opposite) then
                local trades = trading:FindTrade():WhenSide("S"):WhenCustomID(custom_id):All();
                for _, trade in ipairs(trades) do
                    trading:Close(trade);
                end
            end
            local command = trading:MarketOrder(source:instrument()):SetSide("B"):SetDefaultAmount():SetCustomID(custom_id);
